...
def prep_paths_for_save(paths: Dict[str, Any], path: str, i: int) -> Dict[
    str, Any]:
    if path in paths:
        paths[path].append(i)
    else:
...
def save_specification(spec: Dict[str, Any],
                       filename: str = 'specifications.json') -> None:
...
def load_specification(hash_value: str,
                       filename: str = 'specifications.json') -> Dict[
    str, Any]:
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            specs = json.load(f)
        ...
        print(
            f"Error: The file '{filename}' was not found or contains invalid JSON.")
        return {}
